...
import { useOrgType, useCurrentOrg } from "@/hooks/use-auth";
...
export function EmptyPropertyState() {
  ...
  const isCompanyClient = orgType === "COMPANY" && currentOrg?.is_client === true;
  const canAddProperty = !isCompanyClient;
  ...
  return (
    <EmptyState
      title={canAddProperty ? "No properties yet" : "No properties shared with you"}
      subtitle={canAddProperty ? "Create your first property to get started." : "Properties shared with you will appear here."}
    >
      {canAddProperty && (
        <>
